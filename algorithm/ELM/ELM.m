...
clear a;                                   %   Release raw training data array
...
clear b;                                    %   Release raw testing data array
...
if size(H, 1)<=size(H, 2);
    tic;
%	beta = (X*X'+C1*eye(size(X, 1)))\(X*temp_T'-C2*sign(temp_T'/X'));
    beta=(eye(size(H, 1))/C+H*H')\(H*T');
    predict_Y=beta'*H;
%    predict_Y=beta*H;
    train_time=toc;
 else
    tic;
    beta=H*((eye(size(H, 2))/C+H'*H)\(T'));
%	beta = (X*temp_T'-(C2*sign(temp_T'\X'))')\(X*X'+C1*eye(size(X, 1)));        %beta 38*18000
    predict_Y=beta'*H;     
%predict_Y=beta*H;
     train_time=toc;
end
...
%Y = beta*H;
if Elm_Type == REGRESSION
    TrainingAccuracy=sqrt(mse(T - predict_Y))               %   Calculate training accuracy (RMSE) for regression case
end
clear H;
...
%%%%%%%%%% Calculate training & testing classification accuracy
    ...
    for i = 1 : size(T, 2)
        ...
        [x, label_index_actual]=max(predict_Y(:,i));
